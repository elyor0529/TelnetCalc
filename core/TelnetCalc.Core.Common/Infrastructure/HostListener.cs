...
namespace TelnetCalc.Core.Common
{
    ...
    /// the <see cref="AppFactory"/> class is processing it. Once that completes,
    ...
    public class HostListener
    {
        ...
        private static ILog _log = LogManager.GetLogger(typeof(HostListener));

        public HostListener(int port, int maxConnections)
        {
            _port = port;
